...
// Onboarding sample prompts (shown in the empty chat + welcome modal).
const SAMPLE_CHIPS = [
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const u = getUser();
    setUser(u);
    // First-time onboarding: shown once per user (tracked in localStorage).
    const key = `agi_onboarded_${u?.id ?? "anon"}`;
    if (u && !localStorage.getItem(key)) setShowOnboarding(true);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  function finishOnboarding() {
    ...
    if (user) localStorage.setItem(`agi_onboarded_${user.id}`, "1");
  }
  ...
  async function decide(index: number, decision: "approved" | "rejected") {
    ...
    try {
      ...
      const content =
        data?.reply || (decision === "approved" ? "✅ Done." : "✕ Cancelled.");
  ...
  async function decideBulk(index: number, decision: "approve" | "reject") {
    ...
    try {
      const data = await apiJson<{
        ...
        failed?: number;
        total?: number;
      }>(`${API_URL}/api/bulk/${decision}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ bulk_id: msg.bulk.bulk_id }),
      });
      // Build the outcome message from the counts — a 200 response is a
      // success even if some individual items failed.
      let content: string;
      if (decision === "reject") {
        content = data?.reply || "✕ Cancelled — nothing was sent.";
      } else {
        const success = data?.success ?? 0;
        const failed = data?.failed ?? 0;
        const total = data?.total ?? success + failed;
        content =
          failed > 0
            ? `⚠️ ${success} sent, ${failed} failed.`
            : `✅ ${success}/${total} sent successfully.`;
      }
      setMessages((prev) => [...prev, { role: "assistant", content }]);
    } catch (err) {
      // Only a real network / non-200 failure lands here.
      setMessages((prev) => prev.map((m, i) => (i === index ? { ...m, decided: undefined } : m)));
      setMessages((prev) => [...prev, { role: "error", content: errMessage(err) }]);
    } finally {
      setLoading(false);
    ...
    <main className="flex min-h-full flex-col bg-[#050807] font-sans text-emerald-50/90">
      ...
      {showOnboarding && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-emerald-500/25 bg-[#070c0a] p-6 shadow-2xl shadow-emerald-500/10">
            <div className="mb-3 flex items-center gap-2">
              <span className="text-xl">✦</span>
              <h2 className="text-lg font-bold text-emerald-300">Welcome to AGI-CORE! 👋</h2>
            </div>
            <p className="mb-4 text-sm text-emerald-100/70">
              I&apos;m your AI business assistant. I can query your data, make reports and charts,
              send emails, and more. Let me show you what I can do.
            </p>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-emerald-200/40">
              Try asking me:
            </p>
            <div className="mb-5 space-y-1.5">
              {SAMPLE_CHIPS.slice(0, 4).map((chip) => (
                <button
                  key={chip}
                  onClick={() => {
                    finishOnboarding();
                  ...
                  className="block w-full rounded-lg border border-emerald-500/15 bg-emerald-500/[0.05] px-3 py-2 text-left text-sm text-emerald-100/80 transition hover:border-emerald-400/40 hover:bg-emerald-500/10"
                >
                  &ldquo;{chip}&rdquo;
                </button>
              ))}
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className="text-xs text-emerald-200/40 transition hover:text-emerald-200/70"
              >
                Skip
              </button>
              <button
                onClick={finishOnboarding}
                className="rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 px-5 py-2 text-sm font-semibold text-black shadow-lg shadow-emerald-500/25 transition hover:from-emerald-300 hover:to-teal-400 active:scale-95"
              >
                Let&apos;s Go
              </button>
            </div>
        ...
        {messages.length === 0 && !loading && (
          <div className="mt-6 flex flex-col items-center gap-4">
            <p className="text-center text-sm text-emerald-200/50">
              Ask me anything about your business — I&apos;ll route the work through the core.
            </p>
            <div className="flex flex-wrap justify-center gap-2">
              {SAMPLE_CHIPS.map((chip) => (
                <button
                  ...
                  className="rounded-full border border-emerald-500/25 bg-emerald-500/[0.06] px-3.5 py-1.5 text-xs text-emerald-200/80 transition hover:border-emerald-400/50 hover:bg-emerald-500/12 active:scale-95"
                >
                  {chip}
                </button>
              ))}
